...
function settingsBindCollections(root, profile) {
  const setColl = (key, value) => {
    profile.recipe_collections = Object.assign({ nhs: true, parentclub: true, nhlbi: true, wikibooks: true, usda: false }, profile.recipe_collections || {}, { [key]: value });
    uiPersist();
  ...
  for (const key of ['wikibooks', 'nhs', 'parentclub', 'nhlbi']) {
